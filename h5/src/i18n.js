...
const messages = {
  'zh-CN': {
    ...
    // 多媒体附件
    'media.image.too.large': '图片 {name} 超过 5MB 限制',
    'media.video.too.large': '视频 {name} 超过 50MB 限制',
    'media.video.error': '视频 {name} 处理失败，请检查格式是否支持',
    'media.video.context': '[视频分析] 用户上传了视频文件 "{name}"{dur}，已提取 {frames} 帧关键截图供视觉分析。请结合图像内容和用户的提问，描述视频中发生的事件和可见细节。',
    'media.video.context.dur': '（{dur}）',
    'media.video.seconds': '秒',
    'media.video.frames': '帧',
    // 会话管理
    'session.title': '会话管理',
    'session.new': '新建会话',
    ...
    // 跨设备同步
    'cmd.sync': '跨设备同步',
    'cmd.sync.run': '同步消息到所有已连接设备（输入内容）',
    'sync.title': '跨设备同步',
    'sync.received': '来自其他设备的同步',
    'sync.sent': '已送达 {n} 台设备',
    'sync.sent.none': '当前没有其他已连接设备',
    'sync.receipt.total': '共',
    'sync.error': '同步失败，请检查连接',
    // 引导
  ...
  'en': {
    ...
    // Media attachments
    'media.image.too.large': 'Image {name} exceeds 5 MB limit',
    'media.video.too.large': 'Video {name} exceeds 50 MB limit',
    'media.video.error': 'Failed to process video {name}. Please check that the format is supported.',
    'media.video.context': '[Video Analysis] The user uploaded a video file "{name}"{dur} and {frames} key frames have been extracted for visual analysis. Please describe the events and visible details in the video based on the frames and the user\'s question.',
    'media.video.context.dur': ' ({dur})',
    'media.video.seconds': 's',
    ...
    // Cross-device sync
    'cmd.sync': 'Cross-device Sync',
    'cmd.sync.run': 'Sync message to all connected devices (enter content)',
    'sync.title': 'Cross-device Sync',
    'sync.received': 'Sync from another device',
    'sync.sent': 'Delivered to {n} device(s)',
    'sync.sent.none': 'No other connected devices',
    'sync.receipt.total': 'of',
    'sync.error': 'Sync failed, please check your connection',
