...
const messages = {
  'zh-CN': {
    ...
    // 设备控制
    'device.context': '[本地设备助手模式]\n当前设备可用操作：{caps}\n\n设备控制规则：\n• 白名单操作：vibrate（震动）、setVolume:50（音量0-100）、playMedia:URL（播放媒体）、stopMedia（停止媒体）、launchApp:URL（打开应用）、setBrightness:80（亮度0-100）\n• 执行白名单且非敏感操作时，在回复中嵌入：[ACTION: 操作名:参数]，然后输出友好结果文字（如"已震动提醒。"）\n• 执行敏感操作（setDoNotDisturb）时，使用：[CONFIRM: 操作名:参数]，等待用户授权\n• 不在白名单内的操作：礼貌拒绝，不输出任何代码或执行细节\n• 已明确禁止的操作：未经用户同意的持续麦克风监听、摄像头调用、屏幕录制、第三方 App 自动操作\n---\n',
    'device.settings.label': '🤖 设备控制助手',
    'device.settings.on': '启用',
    'device.settings.off': '关闭',
    'device.settings.hint': '开启后 AI 可在回复中发出震动、音量调节、应用启动等设备指令。敏感操作（勿扰模式等）需要你逐次确认后才执行。',
    'device.confirm.title': '敏感操作需要授权',
    'device.confirm.btn': '允许执行',
    'device.confirm.cancel': '拒绝',
    'device.result.vibrate': '已震动提醒。',
    'device.result.setvolume': '已将音量调至 {level}%。',
    'device.result.launchapp': '已打开应用。',
    'device.result.setbrightness': '已将亮度调至 {level}%。',
    'device.result.playmedia': '已开始播放。',
    'device.result.stopmedia': '已停止播放。',
    'device.result.dnd.on': '已开启勿扰模式。',
    'device.result.dnd.off': '已关闭勿扰模式。',
    'device.error.not_supported': '当前设备不支持此操作。',
    'device.error.not_whitelisted': '此操作不在允许列表内，已拒绝。',
    'device.error.invalid_param': '操作参数无效，请重试。',
    'device.error.launch_failed': '应用启动失败。',
    'device.error.media_failed': '媒体播放失败。',
    'device.error.brightness_failed': '亮度调节失败。',
    // 引导
  ...
  'en': {
    ...
    // Device control
    'device.context': '[Local Device Assistant Mode]\nAvailable device capabilities: {caps}\n\nDevice control rules:\n• Whitelisted actions: vibrate, setVolume:50 (0-100), playMedia:URL, stopMedia, launchApp:URL, setBrightness:80 (0-100)\n• For whitelisted non-sensitive actions, embed in reply: [ACTION: name:params], then output a friendly result (e.g. "Vibration sent.")\n• For sensitive actions (setDoNotDisturb), embed: [CONFIRM: name:params] and wait for user authorization\n• Actions not on the whitelist: politely decline, never output code or execution details\n• Absolutely prohibited: continuous mic listening, camera access, screen recording, or autonomous third-party app operations without explicit user consent\n---\n',
    'device.settings.label': '🤖 Device Control Assistant',
    'device.settings.on': 'Enabled',
    'device.settings.off': 'Disabled',
    'device.settings.hint': 'When enabled, the AI can issue device commands (vibrate, volume, app launch, etc.) in its replies. Sensitive actions (Do Not Disturb etc.) require your explicit confirmation each time.',
    'device.confirm.title': 'Sensitive action requires authorization',
    'device.confirm.btn': 'Allow',
    'device.confirm.cancel': 'Deny',
    'device.result.vibrate': 'Vibration sent.',
    'device.result.setvolume': 'Volume set to {level}%.',
    'device.result.launchapp': 'App launched.',
    'device.result.setbrightness': 'Brightness set to {level}%.',
    'device.result.playmedia': 'Playback started.',
    'device.result.stopmedia': 'Playback stopped.',
    'device.result.dnd.on': 'Do Not Disturb enabled.',
    'device.result.dnd.off': 'Do Not Disturb disabled.',
    'device.error.not_supported': 'This operation is not supported on the current device.',
    'device.error.not_whitelisted': 'This action is not on the allowed list and was denied.',
    'device.error.invalid_param': 'Invalid parameters, please try again.',
    'device.error.launch_failed': 'Failed to launch app.',
    'device.error.media_failed': 'Media playback failed.',
    'device.error.brightness_failed': 'Failed to adjust brightness.',
